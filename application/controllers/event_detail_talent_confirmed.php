...
error_reporting(0);
class event_detail_talent_confirmed extends CI_Controller {
...
public function __construct()
	...
	public function index()
	{
		...
		if($myuser_id!="") {
			...
			//$my_event['hired_info']= $this->hired_info_model->index($myuser_id,$event_id);	
			//$data['events'] = $this->invite_model->invite_details($result);
			//print_r($my_event['event_detail']);
			$this->load->view('event_detail_talent_confirmed',$my_event);
		}
		else {
			redirect('login');
		}		
