...
error_reporting(0);
class client_proposals_applied extends CI_Controller {
...
public function __construct()
	...
	public function index()
	{
		...
		if($myuser_id!="") {
			...
			$this->load->view('client_proposals_applied',$my_event);  
		}
		else {
			redirect('login');
		}
	...
	{    
		 if($_POST) 
			{	
				...
				$query=$this->db->query("SELECT * from invite_talent_to_event WHERE status = 5 AND client_id = ".$myuser_id." AND event_id = ".$event_id." ORDER BY invite_id ASC LIMIT $position, $items_per_group");			
				...
				//print_r($data['blogs']);
				$this->load->view('client_proposals_applied_list_view',$data);
