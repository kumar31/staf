...
$(document).ready(function() {
	...
	$('#results').load("http://smaatapps.com/nector/website/index.php/management/getblogdata/<?php echo $event_id ?>", {'group_no':track_load}, function() {track_load++;}); //load first group
	...
	$(window).scroll(function() { //detect page scroll
		...
		if($(window).scrollTop() + $(window).height() == $(document).height())  //user scrolled to bottom of the page?
		{
			...
			if(track_load <= total_groups && loading==false) //there's more data to load
			{
				...
				$.post('http://smaatapps.com/nector/website/index.php/management/getblogdata/<?php echo $event_id ?>',{'group_no': track_load}, function(data){
        ...
        <div class="col-sm-4 topmargin listings-invitebar">
          <section class="clearfix whiteBG classWithPadLeft">
                ...
                <h3 data-placement="bottom" data-toggle="tooltip" title="This is the estimated price for your event based on the hours and numbers of talent needed. Should this change during the event we will automatically add or refund the amended amount." id="number">$<?php /*$total_hours_est = $hrs_mins;
					...
					$per_talent_amt = "";
						...
						{		
								$talent = $val['talent'][0];
								$talent['talent_id'];
								$checkout_not_paid_detail = $val['checkout_not_paid_detail'];
			...
			<?php 
				...
				?>
				<?php
					 $number_of_talents = $event_detail[0]['number_of_waiters'];
						...
						if($amount > $per_talent_amt) {
							$pay_amt = $amount - $per_talent_amt;
						}
						
						else  {
							$pay_amt = $per_talent_amt - $amount;							
						}
						
						if($pay_amt > 0) { ?>
						<input type="hidden" value="<?php echo $pay_amt; ?>" id="apply<?php echo $event_detail[0]['event_id']; ?>">
							 <a class="btn btn-danger btn-lg largeHeight" value="<?php echo $pay_amt; ?>" onclick="applytolaunch('<?php echo $pay_amt; ?>','<?php echo $event_detail[0]['event_id']; ?>','<?php echo $myuser_id; ?>','<?php echo $total_talents; ?>')"
							 role="button" id="btn-paid<?php echo $event_detail[0]['event_id']; ?>">Pay remaining $<?php echo $pay_amt; ?> & Launch Event</a>
					<?php 	} else {  ?>
	...
	<script>
		...
		function delevent(){
			...
			var event_id = <?php echo $event_detail[0]['event_id']; ?>; 
				...
				var url = 'http://smaatapps.com/nector/webservice/index.php/delete_event';
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(message == "1") {
							window.location.assign("http://smaatapps.com/nector/website/index.php/myevents_client");
						}
						else {
		...
		function closeevent(){
			...
			var event_id = <?php echo $event_detail[0]['event_id']; ?>; 
				...
				var url = 'http://smaatapps.com/nector/webservice/index.php/event_close';
		...
		function launchevent(){
			...
			var event_id = <?php echo $event_detail[0]['event_id']; ?>; 
				...
				var url = 'http://smaatapps.com/nector/webservice/index.php/launch_event';
		...
		function applytolaunch(textbox_val,event_id,client_id,total_talents){
		  ...
		  $(btn).attr('disabled', true);
			...
			var amount = textbox_val; 
				...
				var url = 'http://smaatapps.com/nector/stripe/payment_launch.php';
		...
		function refundamount(textbox_val,event_id,client_id,total_talents){
		  ...
		  $(btn).attr('disabled', true);
			...
			var amount = textbox_val; 
				...
				var url = 'http://smaatapps.com/nector/stripe/refund.php';
