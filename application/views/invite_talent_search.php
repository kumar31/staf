...
$(document).ready(function() {
	...
	<?php if(isset($_POST['eventid'])){ ?>
		var eventid = '<?php echo $_POST['eventid']; ?>';
	<?php }else{ ?>
		var eventid = '';
	<?php } ?>
	...
	$('#results').load("http://smaatapps.com/nector/website/index.php/invite_talent_search/getblogdata/<?php echo $event_id; ?>", {'group_no':track_load,'search':search,'special_skills':special,'hair_color':haircolor,'eye_color':eyecolor,'languages':language,'height_min':heightfrom,'height_max':heightto,'best_rating':sort,'gender':gender,'eventid':eventid}, function() {track_load++;}); //load first group
	...
	$(window).scroll(function() { //detect page scroll
		...
		if($(window).scrollTop() + $(window).height() == $(document).height())  //user scrolled to bottom of the page?
		{
			...
			if(track_load <= total_groups && loading==false) //there's more data to load
			{
				...
				$.post('http://smaatapps.com/nector/website/index.php/invite_talent_search/getblogdata/<?php echo $event_id; ?>',{'group_no': track_load,'search':search,'special_skills':special,'hair_color':haircolor,'eye_color':eyecolor,'languages':language,'height_min':heightfrom,'height_max':heightto,'best_rating':sort,'gender':gender,'eventid':eventid}, function(data){
  ...
  <body>
    <div class="container">
      <div class="row" style="margin-bottom: 200px; margin-top:20px;">
		...
		<a class="btn btn-danger btn-block" href="<?php echo site_url();?>/create_event" role="button">Create event</a>
	  <hr>
          <section class=
                   "clearfix hourlies-listing-sidebar listings-sidebar greyGB" id=
                   "listing-sidebar">
			<form action="<?php echo base_url();?>index.php/invite_talent_search/<?php echo $event_id; ?>" method="POST">
            ...
            <div class="skills">
              ...
              <div class="checkbox checkbox-warning">
					<input id="checkbox1" onclick="this.form.submit()" name="special" class="styled" type="checkbox" value="bartending" <?php if(isset($_POST['special'])) { if($_POST['special'] == "bartending") { echo "checked";} }  ?>>
					<label for="checkbox1">
						Bartending
					</label>
              </div>
			  ...
			  <div class="checkbox checkbox-warning">
					<input id="checkbox2" onclick="this.form.submit()" name="special1" class="styled" type="checkbox" value="waiter,waitress" <?php if(isset($_POST['special1'])) { if($_POST['special1'] == "waiter,waitress") { echo "checked";} }  ?>>
					<label for="checkbox2">
						Waiter/waitress
					</label>
              </div>
			  ...
			  <div class="checkbox checkbox-warning">
					<input id="checkbox3" onclick="this.form.submit()" name="special2" class="styled" type="checkbox" value="personality,host" <?php if(isset($_POST['special2'])) { if($_POST['special2'] == "personality,host") { echo "checked";} }  ?>>
					<label for="checkbox3">
						Personality/host
					</label>
              </div>
            ...
            <div class="gender">
              ...
              <hr>
              <div class="checkbox checkbox-warning">
					<input onclick="this.form.submit()" id="checkboxe" name="gender" class="styled" type="checkbox" value="0,1" <?php if(isset($_POST['gender'])) { if($_POST['gender'] == "0,1") { echo "checked";} }  ?>>
					<label for="checkboxe">
						Both
					</label>
              </div>
              <div class="checkbox checkbox-warning">
					<input onclick="this.form.submit()" id="checkboxm" name="gender1" class="styled" type="checkbox" value="1" <?php if(isset($_POST['gender1'])) { if($_POST['gender1'] == "1") { echo "checked";} }  ?>>
					<label for="checkboxm">
						Male
					</label>
              </div>
              <div class="checkbox checkbox-warning">
					<input onclick="this.form.submit()" id="checkboxf" name="gender2" class="styled" type="checkbox" value="0" <?php if(isset($_POST['gender2'])) { if($_POST['gender2'] == "0") { echo "checked";} }  ?>>
					<label for="checkboxf">
						Female
					</label>
              </div>
