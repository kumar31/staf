                    ...
                    <div class="col-xs-8 col-sm-6">
					  ...
					  <?php $checkout_agreed_detail = $val['checkout_agreed_detail']; 
							...
							if(($checkout_agreed_detail == 0) && ($checkout_recheck_detail == 1)) { ?>
								 <h5 class="text-danger"><b>Talent requested time &nbsp;-&nbsp; Days <?php echo $val['check_details_talent'][0]['number_of_days']; ?> &nbsp;Hours &nbsp;<?php echo $val['check_details_talent'][0]['number_of_hours']; ?> &nbsp;Minutes &nbsp;<?php echo $val['check_details_talent'][0]['number_of_minutes']; ?> </b></h5>
							<?php }
					   ?>
					 ...
					 <div class="col-xs-12 col-sm-3"> 					
					   <div class="checkbox-warning centerText"> 
						...
						<input type="hidden" value="<?php echo $talent['talent_id']; ?>" id="reject<?php echo $talent['talent_id']; ?>">					  
							<a class=
						   "btn btn-submit btn-lg largeHeight once-only btn_confirms disabled" value="<?php echo $talent['talent_id']; ?>"
						...
						<input type="hidden" value="<?php echo $talent['talent_id']; ?>" id="reject<?php echo $talent['talent_id']; ?>">					  
						   ...
						   role="button" id="btn2" textbox_id="#reject<?php echo $talent['talent_id']; ?>">Check In Now
						 </a>							
						<? } ?>
					...
					<div class="modal-dialog">
					  ...
					  <div class="modal-content text-center">
						...
						<input type="hidden" value="<?php echo $talent['talent_id']; ?>" id="cout<?php echo $talent['talent_id']; ?>">					  
						   ...
						   role="button" id="btn2" textbox_id2="#cout<?php echo $talent['talent_id']; ?>">Yes
						 </a>
...
<script>
 <?php foreach($events as $val) {
	 ...
	 {	
		...
		checkoutnow<?php echo $talent['talent_id']; ?>(textbox_val2);
		//$("#myModal").modal("hide");
	 });
	
  function checkoutnow<?php echo $talent['talent_id']; ?>(textbox_val2){
		...
		//alert(number_of_minutes); die;
		
			var url = 'http://smaatapps.com/nector/webservice/index.php/talent_checkout';
			
			$.ajax({
				'type' : 'POST',
				'url': url,
				'data': {'talent_id':talent_id,'event_id':event_id,'client_id':client_id,'number_of_days':number_of_days,'number_of_hours':number_of_hours,'number_of_minutes':number_of_minutes,'talent_rating':rating},
				//'dataType': 'json',
...
<script>
  ...
  function applytoevent(textbox_val){
		...
		var client_id = <?php echo $myuser_id; ?>; 
			...
			var url = 'http://smaatapps.com/nector/webservice/index.php/talent_checkin';
