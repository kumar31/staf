		...
		<div class="prepend-top clearfix form-group">
		  ...
		  <label for="" class="required">Profile Picture
			...
			<?php
					...
					<img class="img-circle center-block pimg" style="width: 120px; height: 120px;" src="">
					<br>
				<?php				
				}
				?>
  ...
  <script>
		...
		function reg(){
			...
			if(typeof gender === "undefined") {
				...
				var url = 'http://smaatapps.com/nector/webservice/index.php/talent_registration';
				
				$.ajax({
					'type' : 'POST',
					'url': url,
					'data': {'email':email,'password':password,'first_name':firstname,'last_name':lastname,'phone_number':phone,
					'gender':gender,'address':address,'dob':dob,'profile_url':profile_url,'w9_form':w9_form,'special_skills':skills,
					'latitude':lat,'longitude':lng,'longitude':lng,'longitude':lng,'longitude':lng,'facebook_id':facebook_id,'login_type':login_type,'date':date},
					//'dataType': 'json',
					beforeSend: function(){
					 $(".se-pre-con").show();
				   },
